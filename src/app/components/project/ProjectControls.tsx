...
export function ProjectControls({ allProjects, onFilterChange }: ProjectControlsProps) {
  ...
  return (
    <div className="py-4 space-y-4 bg-background sticky top-0 z-10">
      {/* Search and Sort Row */}
      <div className="flex flex-col sm:flex-row gap-4">
        <input
          type="text"
          placeholder="Search projects..."
          className="flex-grow p-2 border border-secondary rounded-lg bg-background text-text focus:ring-primary focus:border-primary"
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
        />
        <select
          className="p-2 border border-secondary rounded-lg bg-background text-text focus:ring-primary focus:border-primary sm:w-48"
          value={sortOrder}
      ...
      <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className={`px-3 py-1 border rounded-full text-xs transition-colors
              ${selectedTags.includes(tag) 
                ...
                : 'bg-background border-secondary text-text hover:border-primary'}`}
          >
            {tag}
          </button>
        ))}
        {selectedTags.length > 0 && (
          <button
            ...
            className="px-3 py-1 text-xs text-textMuted hover:text-text underline"
          >
            Clear filters
          </button>
        )}
